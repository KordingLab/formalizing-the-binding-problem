...
class VGTopAttrDataset(BindingDataset):
    """
    Visual Genome: Top attributes binding dataset built from mined json/jsonl.
    """
    ...
    def __init__(self, cfg, seed=0, num_samples=None, transform=None, output_dir=None, split="train"):
        ...
        meta_dir = os.path.join(self.data_dir, "deprecated", f"meta_filtered_topattr_{metadata_split}")

        # ---- load files ----
        meta_path = os.path.join(meta_dir, "attributes_filtered_topattr.json")
        cat_attr_path = os.path.join(meta_dir, "vg_stats_filtered_topattr.json")
        ...
        attributes = cat_attr.get("selected_topattr_attributes", [])

        if "no_attribute" not in attributes:
            attributes = list(attributes) + ["no_attribute"]
        ...
        if isinstance(attr_occ, dict) and len(attr_occ) > 0:
            attributes = sorted(attributes, key=lambda a: (-attr_occ.get(a, 0), a))

        self.categories = categories
        self.attributes = attributes
        self.cat2i = {c: i for i, c in enumerate(categories)}
        self.col2i = {c: i for i, c in enumerate(attributes)}


        super().__init__(
            feature_groups=[categories, attributes],
        ...
        N1, N2 = len(categories), len(attributes)
        self.conjunction_statistics = torch.zeros((N1, N2), dtype=torch.float64)

        obj2attr = cat_attr.get("object_to_topk_attribute_occurrences", {})  # {obj: {attr: count}}
        total = 0.0
        ...
        if len(self.Y) == 0:
            raise RuntimeError(f"No VGTopAttr samples found for split={self.split!r} under {meta_dir}.")
...
def metadata_to_features(
    ...
    *,
    ...
    for o in objs:
        ...
        attrs = o.get("attributes_topattr", []) or []
        chosen = None
        for a in attrs:
